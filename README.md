...
1. 如果命令行里面有难以记忆的部分，并且也需要经常使用它，可使用gurl -gen &>demo.cf.lua 命令保存到文件里
2. gurl -K demo.cf.lua根据配置文件里面的数据访问服务端
3. 如果要修改demo.cf.lua，也不熟悉配置文件的写法，可以通过./gurl -K demo.cf.lua -gen 命令把demo.cf.lua的数据转成命令行，然后重复第1步操作
